...
function fixPathsInFile(filePath) {
  ...
  if (content.match(imagePattern)) {
    content = content.replace(imagePattern, '![$1]({{ \'/images/$2\' | relative_url }})');
    modified = true;
  }
  ...
  if (content.match(filePattern)) {
    content = content.replace(filePattern, '[$1]({{ \'/files/$2\' | relative_url }})');
    modified = true;
  }
  ...
  if (content.match(backtickFilePattern)) {
    content = content.replace(backtickFilePattern, '`{{ \'/files/$1\' | relative_url }}`');
    modified = true;
  }
